...
st.set_page_config(
    ...
    menu_items={
        ...
        "automatically generate research-backed scripts, match video materials, "
        "synthesize speech narration, create subtitles, and produce high-definition videos.",
    },
)
...
def _infer_tts_server_from_voice(voice_name):
    ...
    if voice.is_edge_tts_voice(voice_name):
        return "edge-tts"
...
def _render_brand(available_update: str | None = None):
    ...
    st.markdown(
        f"""
        <h1 class="mpt-brand">
            <span class="mpt-brand__name">HTvideoAI</span>
...
def _render_script_settings(panel, params):
    ...
    with panel:
        with st.container(border=True):
            ...
            duration_options = {
                0: "Tự động theo số đoạn",
                30: "30 Giây (Shorts)",
                60: "1 Phút (60s)",
                120: "2 Phút (120s)",
                180: "3 Phút (180s - Long Video)",
                300: "5 Phút (300s - Long Video)",
                600: "10 Phút (600s - Documentary)",
                900: "15 Phút (900s - Feature)",
            }
            selected_duration = st.selectbox(
                "Thời lượng video tổng ước tính (Target Video Duration)",
                options=list(duration_options.keys()),
                format_func=lambda x: duration_options[x],
                key="target_duration_seconds_input",
            )
            params.target_duration_seconds = selected_duration

            params.voice_style = st.text_input(
                "Phong cách giọng đọc Custom (Voice Style Prompt)",
                value=getattr(params, "voice_style", ""),
                placeholder="Ví dụ: nam trầm, bình tĩnh, truyền cảm",
                key="voice_style_input",
            ).strip()

            params.enable_research = st.checkbox(
                "Bật Research Pipeline (Tự động tìm kiếm web trước khi viết kịch bản)",
                value=getattr(params, "enable_research", False),
                key="enable_research_input",
            )

            # 使用带 key 的局部容器限定折叠入口样式，保持 expander 的原生交互，
            # 同时避免样式误伤页面顶部的“基础设置”等其他折叠区域。
            with st.container(key="advanced_settings_script"):
                with st.expander(tr("Advanced Script Settings"), expanded=True):
                    ...
                    if preview_prompt_col.button(
                        ...
                        render_script_prompt_preview(
                            llm.build_script_prompt(
                                ...
                                duration_seconds=getattr(params, "target_duration_seconds", 0),
                            )
                        )
    ...
    with panel:
        with st.container(border=True):
            ...
            if not tts_mode_enabled:
                ...
                filtered_voices = voice.get_edge_tts_voices()
