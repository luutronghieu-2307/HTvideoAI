...
MAX_SCRIPT_PARAGRAPH_NUMBER = 50
    ...
    for i in range(_max_retries):
        try:
            ...
            if response and response.startswith("Error: "):
                if any(kw in response.lower() for kw in ["503", "unavailable", "429", "rate_limit", "high demand", "busy"]):
                    logger.warning(f"LLM provider busy/unavailable (503/429), auto backing off 8s... (try {i + 1}/{_max_retries}): {response}")
                    time.sleep(8)
                    continue
                else:
            ...
            if final_script and not final_script.startswith("Error: "):
                break
        except Exception as e:
...
def generate_terms(
    ...
    truncated_subject = _limit_script_text(video_subject, 200, "video_subject")
    truncated_script = _limit_script_text(video_script, 1000, "video_script")
...
""".strip()
    ...
    for i in range(_max_retries):
        try:
            ...
            if response.startswith("Error: "):
                # Nếu gặp lỗi Rate Limit 429 (vượt quá token/phút của Groq/OpenAI free tier),
                # tự động chờ 10-15s để nguội quota rồi thử lại thay vì thất bại ngay lập tức.
                if any(err_kw in response.lower() for err_kw in ["429", "rate_limit", "rate limit", "too many requests", "tokens per minute"]):
                    logger.warning(f"LLM rate limit (429) hit, auto backing off 12s... (try {i + 1}/{_max_retries}): {response}")
                    time.sleep(12)
                    continue
