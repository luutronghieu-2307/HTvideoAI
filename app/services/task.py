...
def generate_script(task_id, params):
    ...
    if not video_script:
        source_notes = ""
        enable_research = getattr(params, "enable_research", False)
        if enable_research:
            try:
                from app.services.research import SearxngResearchBackend
                research_backend = SearxngResearchBackend()
                candidates = research_backend.discover(params.video_subject)
                evidence = research_backend.retrieve(params.video_subject)
                source_notes = "\n".join([f"- {item.excerpt}" for item in evidence if item.excerpt])
                logger.info(f"Retrieved research notes: {len(evidence)} excerpts")
            except Exception as exc:
                logger.warning(f"Research retrieval warning: {exc}")
        ...
        video_script = llm.generate_script(
            ...
            audience=getattr(params, "audience", ""),
