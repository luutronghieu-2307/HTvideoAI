...
class TestVoiceService(unittest.TestCase):
    ...
    def test_custom_edge_tts_voice_detection_and_dispatch(self):
        """
        验证 custom_edge_tts 模块 detect, parse, list 以及 tts 调度。
        """
        self.assertTrue(vs.is_edge_tts_voice("edge_tts:zh-CN-XiaoxiaoNeural"))
        self.assertTrue(vs.is_edge_tts_voice("edge:zh-CN-YunxiNeural-Male"))
        self.assertFalse(vs.is_edge_tts_voice("zh-CN-XiaoxiaoNeural"))

        self.assertEqual(
            vs.parse_edge_tts_voice_name("edge_tts:zh-CN-XiaoxiaoNeural-Female"),
            "zh-CN-XiaoxiaoNeural",
        )
        self.assertEqual(
            vs.parse_edge_tts_voice_name("edge:zh-CN-YunxiNeural-Male"),
            "zh-CN-YunxiNeural",
        )

        edge_voices = vs.get_edge_tts_voices()
        self.assertTrue(len(edge_voices) > 0)
        self.assertTrue(all(v.startswith("edge_tts:") for v in edge_voices))

        sentinel = object()
        with patch.object(vs, "custom_edge_tts", return_value=sentinel) as mock_edge:
            result = vs.tts(
                text="testing edge_tts dispatch",
                voice_name="edge_tts:zh-CN-XiaoxiaoNeural-Female",
                voice_rate=1.0,
                voice_file="/tmp/edge-tts-test.mp3",
            )
            self.assertIs(result, sentinel)
            mock_edge.assert_called_once_with(
                "testing edge_tts dispatch",
                "edge_tts:zh-CN-XiaoxiaoNeural-Female",
                1.0,
